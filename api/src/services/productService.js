...
class ProductService {
  ...
  // ------------------------------------------------------------------
  // Public Methods (Converted to PostgreSQL)
  // ------------------------------------------------------------------
  ...
  async getProducts(filters = {}) {
    ...
    try {
      ...
      const productsQuery = `
        ...
        ${whereClause}
        GROUP BY p.product_id, c.category_id, c.name, p.name, p.description, p.price, p.stock_quantity, p.sku, p.image_url, p.brand, p.created_at
  ...
  async updateProduct(productId, productData) {
    ...
    const updateTime = new Date().toISOString(); // PostgreSQL requires explicit timestamp update

    try {
      const updateQuery = `
        UPDATE products 
        SET name = ?, description = ?, category_id = ?, price = ?, stock_quantity = ?, sku = ?, brand = ?, image_url = ?, updated_at = ?
        WHERE product_id = ?
      `;
      const result = await this.executeQuery(updateQuery, [name, description, category_id, price, stock_quantity, sku, brand, image_url, updateTime, productId]);
